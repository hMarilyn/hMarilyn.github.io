			...
			case "password":
				if (check(this.value, passwordRegex)) {
					...
					if (this.value.length >= 6) {
						passwordDom.querySelector("span").innerText =
							"密码太简单，有被盗风险，建议字母+数字的组合";
					}
				}
				break;
			case "repassword":
                    if (check(this.value, passwordRegex)) {
                        repasswordDom.classList.remove("error");
                        repasswordDom.classList.add("success");
                    } else {
                        repasswordDom.classList.remove("success");
                        repasswordDom.classList.add("error");
                        if (this.value.length !== passwordRegex) {
                            repasswordDom.querySelector("span").innerText =
                                "密码不一致";
                        }
                    }
				break;
